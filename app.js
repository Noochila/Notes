  ...
  post.find({}, function (err, founditem) {
    if (!err) {
      if (founditem.length == 0) {
        const post1 = new post({ title: "", description: "" })
        const postarray=[post1]
        res.render("home", { homeContent: homeStartingContent, p: postarray });
      }
      else {

        res.render("home", { homeContent: homeStartingContent, p: founditem });
